 ...
 * 
 * This worker acts as a CORS proxy for fetching HTML from manga sites
 * and an Image Proxy for fetching images while bypassing hotlink protection.
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/proxy') {
      return await handleHtmlProxy(request, url);
    }

    // Image endpoint: /image?u=https://example.com/image.jpg
    if (url.pathname === '/image') {
      return await handleImageProxy(request, url);
    }

    return new Response("Nyora Web Parsers Proxy API. Use /proxy?url=... or /image?u=...", {
      status: 200,
...
async function handleHtmlProxy(request, workerUrl) {
...
async function handleImageProxy(request, workerUrl) {
  const targetUrl = workerUrl.searchParams.get('u');

  if (!targetUrl) {
    return new Response("Missing 'u' parameter", { status: 400, headers: { "Access-Control-Allow-Origin": "*" } });
  }
